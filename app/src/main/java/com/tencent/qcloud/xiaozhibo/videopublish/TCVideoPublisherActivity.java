...
//            req.put("Action", "GetVodSignature");
        ...
        mSignatureListener = new TCPushFetchSignatureListener(this);
        TCHttpEngine.getInstance().post(req, mSignatureListener);
    }
...
//        }
    ...
    private static class TCPushFetchSignatureListener implements TCHttpEngine.Listener {
        ...
        TCPushFetchSignatureListener(TCVideoPublisherActivity activity) {
            mContext = new WeakReference<TCVideoPublisherActivity>(activity);
        }
        @Override
        public void onResponse(int retCode, String retMsg, JSONObject retData) {
            if (retCode == 0 && retData != null) {
                if (mContext == null) return;
                TCVideoPublisherActivity context = mContext.get();
                if (context != null) {
                    try {
                        context.mCosSignature = retData.getString("signature");
                        context.startPublish();
                    } catch (Exception e) {
                        e.printStackTrace();
                        context.mTVPublish.setText("网络连接断开，视频上传失败");
                    }
                }
            }
        }
    ...
    private static class TCPushUploadUGCVideoListener implements TCHttpEngine.Listener {

        private WeakReference<TCVideoPublisherActivity> mContext;
        private String videoId = "";
        private String videoURL = "";
        private String coverURL = "";
        TCPushUploadUGCVideoListener(TCVideoPublisherActivity activity, final String videoId, final String videoURL, final String coverURL) {
            mContext = new WeakReference<TCVideoPublisherActivity>(activity);
        ...
        public void onResponse(int retCode, String retMsg, JSONObject retData) {
            ...
            if (context != null) {
                try {
                    if (retCode == 0) {
                        context.startShare(videoId, videoURL, coverURL);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
